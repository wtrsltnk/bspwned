...
WorldLoader::WorldLoader(const Config& config)
            : mConfig(config), mFaceCount(0), mFaces(NULL), mShaderIndices(NULL)
{
}
...
bool WorldLoader::loadBSP(const char* filename, WorldRenderer& renderer)
{
    ...
    const IData* bsp = this->mConfig.resourceManager->openFile(filename);
    ...
    if (!loadEntities(bsp, header, renderer))
    {
        this->mConfig.resourceManager->closeFile(bsp);
        return false;
    }
    ...
    if (!loadTextures(bsp, header, renderer))
    {
        this->mConfig.resourceManager->closeFile(bsp);
        return false;
    }
    ...
    if (!loadShaders(bsp, header, renderer))
    {
        this->mConfig.resourceManager->closeFile(bsp);
        return false;
    }
    ...
    if (!loadFaces(bsp, header, renderer))
    {
        this->mConfig.resourceManager->closeFile(bsp);
        return false;
    }
    ...
    if (!loadVisiblity(bsp, header, renderer))
    {
        this->mConfig.resourceManager->closeFile(bsp);
        return false;
    }
...
bool WorldLoader::loadEntities(const IData* bsp, const tBSPHeader& header, WorldRenderer& renderer)
{
...
bool WorldLoader::loadTextures(const IData* bsp, const tBSPHeader& header, WorldRenderer& renderer)
{
...
bool WorldLoader::loadShaders(const IData* bsp, const tBSPHeader& header, WorldRenderer& renderer)
{
...
bool WorldLoader::loadFaces(const IData* bsp, const tBSPHeader& header, WorldRenderer& renderer)
{
    // Load the face data from the BSP in a buffer
    tBSPFace* faces = NULL;
...
bool WorldLoader::loadVisiblity(const IData* bsp, const tBSPHeader& header, WorldRenderer& renderer)
{
