...
bool EntityManager::parseFromBSPEntityData(unsigned char* data, int size)
{
...
bool EntityManager::PIMPL::parseEntity(Tokenizer& tok)
{
...
void EntityManager::getPlayerStart(float position[3]) const
{
    for (int i = 0; i < this->getEntityCount(); i++)
    {
        ...
        if (entityname == "info_player_start")
        {
            for (int j = 0; j < entity->valueCount; j++)
            {
                ...
                if (valuename == "origin")
                {
                    sscanf(value.value, "%f %f %f", &position[0], &position[1], &position[2]);
                    return;
                }
            }
        }
